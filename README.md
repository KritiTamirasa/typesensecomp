...
Core user flow
User uploads or takes a photo of their fridge and/or pantry (can be multiple photos).
Vision model extracts a list of detected ingredients from the photo(s).
That ingredient list becomes the query against a Typesense-indexed recipe database, ranked by how many ingredients match.
Results show as recipe cards: name, matched ingredients, missing ingredients, cook time.
User can toggle "willing to buy more groceries" ON.
When ON, for each missing ingredient in a selected recipe, search a grocery price index and return the closest/cheapest match (store name, price, distance).
When OFF, results are filtered/ranked to prioritize recipes with zero or few missing ingredients ("what can I make right now").
...
Vision API (GPT-4o / Claude vision) — photo → ingredient list.
